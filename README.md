# Pillow GSO optimization benchmark

Artemis-importable repo for [python-pillow/Pillow](https://github.com/python-pillow/Pillow)
GSO tasks. **Edit only `project/`**.
...
```bash
./compile python-pillow__Pillow-63f398b   # sets .gso_task_id + syncs project/
...
| Script | What it does |
|--------|----------------|
| `bash scripts/test_all_commands.sh` | Full E2E: all tasks compile → benchmark → test → reset + validation |
...
String task IDs (e.g. `python-pillow__Pillow-63f398b`) appear in the robust report and
...
| Index | Task ID | API | Primary file(s) to optimize |
|------:|---------|-----|-----------------------------|
| `0` | `python-pillow__Pillow-63f398b` | `ImagingGetBBox` | `project/src/libImaging/GetBBox.c` |
| `1` | `python-pillow__Pillow-d8af3fc` | `Image.split` | `project/PIL/Image.py` (large historical commit — touches several `PIL/*.py`) |
| `2` | `python-pillow__Pillow-f854676` | `GifImageFile.n_frames` | `project/src/PIL/GifImagePlugin.py` |
| `3` | `python-pillow__Pillow-fd8ee84` | `TiffImageFile.is_animated` | `project/PIL/TiffImagePlugin.py`, `project/PIL/GifImagePlugin.py` |
...
```text
pillow/
  project/           edit Pillow source here (agent-visible)
